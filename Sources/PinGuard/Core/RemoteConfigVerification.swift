...
public struct HMACRemoteConfigVerifier: RemoteConfigVerifier {
    ...
    public func verify(blob: RemoteConfigBlob) -> Bool {
        guard case .hmacSHA256(let secretID) = blob.signatureType else { return false }
        guard let secret = secretProvider(secretID) else { return false }
...
public struct PublicKeyRemoteConfigVerifier: RemoteConfigVerifier {
    ...
    public func verify(blob: RemoteConfigBlob) -> Bool {
        guard case .publicKey(let keyID) = blob.signatureType else { return false }
        guard let key = keyProvider(keyID) else { return false }
