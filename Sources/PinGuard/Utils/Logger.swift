...
public enum PinGuardLogger {

    /// The unified logging subsystem name used for all PinGuard logs.
    public static let subsystem = "PinGuard"

    /// Shared `Logger` instance configured with the `PinGuard` subsystem and
    /// the `core` category. Prefer using `PinGuardLogger.log(_:)` rather than
    /// logging directly with this instance to keep messages consistent.
